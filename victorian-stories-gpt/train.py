...
class DataLoader:
    def __init__(self, b, t, process_rank, num_processes, split):
        ...
        is_sagemaker = 'SM_CHANNEL_TRAINING' in os.environ

        if is_sagemaker:
            data_root = '/opt/ml/input/data/training' if split == 'train' else '/opt/ml/input/data/validation'
        else:
            data_root = 'pg19-dataset'
        ...
        # Load tokens as CPU tensor initially
        self.tokens = self.load_tokens(self.shards[self.current_shard])
        self.pos = self.b * self.t * self.process_rank
        logger.info(f"DataLoader initialized with {len(self.shards)} shards")
...
def train_model(args):
    ...
    try:
        ...
        max_steps = 5722  # completes whole data
        max_lr = 1e-4
        min_lr = max_lr * 0.1
        warmup_steps = 100
...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument('--n_channels', type=int, default=768)
